...
```mermaid
flowchart TD
    A([mvn test\nor docker compose up]) --> B[Maven Surefire\nreads testng-full.xml]
    B --> C[TestNG Engine\nregisters Listeners]
    C --> D[Spawns 2 parallel threads\nparallel=classes]
    ...
    subgraph flow ["Per Thread — identical flow"]
        ...
        S15 -- yes, retries left --> S18[Wait 1s / 2s\nback-off]
        S18 --> S8
        S15 -- retries exhausted --> S17
